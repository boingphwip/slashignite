...
async def on_message(message):
    ...
    if message.content.startswith('$event_test'):
        ...
        all_events_dict[str(embed_message.id)] = {
            'can_attend':[],
            'cannot_attend':[],
            'unsure':[],
            'no_response': initial_list,
            'event_message': embed_message
        }
    ...
    if user.name != 'IgnitionBot':
        ...
        if reaction.message.id in event_status_dict.keys():
            ...
            if str(reaction.emoji) in emoji_reaction_actions.keys():
                ...
                if troubleshooting_flag == True:
                    await reaction.message.channel.send('dict message field contained: ' + ', '.join(all_events_dict[str(reaction.message.id)][update_type]))
                ...
                if troubleshooting_flag == True:
                    await reaction.message.channel.send('dict field now contains: ' + ', '.join(all_events_dict[str(reaction.message.id)][update_type]))
                ...
                if troubleshooting_flag == True:
                    await reaction.message.channel.send('field in original message was: ' + new_embed.fields[idx_event_field].value)
                # grab the existing name of the field that we are indexing
                name_field = new_embed.fields[idx_event_field].name

                #grab inline field option
                inline_field_option = new_embed.fields[idx_event_field].inline

                # collect what we are replacing
                # set the field to the required, updated value
                new_embed.set_field_at(index = idx_event_field, name = name_field, value = ", ".join(value_update), inline = inline_field_option)
                
                if troubleshooting_flag == True:
                    await reaction.message.channel.send('field is going to be set to: ' + new_embed.fields[idx_event_field].value)
    ...
    if user.name != 'IgnitionBot':
        ...
        if reaction.message.id in event_status_dict.keys():
            ...
            if str(reaction.emoji) in emoji_reaction_actions.keys():
                ...
                inline_field_option = new_remove_embed.fields[idx_event_field].inline
                # collect what we are replacing
                # set the field to the required, updated value
                new_remove_embed.set_field_at(index = idx_event_field, name = name_field, value = ", ".join(value_update), inline = inline_field_option)
